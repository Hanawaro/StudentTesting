...
void First::set_sort_up(void) {
    ...
    for (int i = 0; i < all_users.size(); i++)
        tmp.push_back(getMark(all_users[i].student.first));
    ...
    for (int i = 0; i < tmp.size(); i++) {
        for (int j = 0; j < all_users.size(); j++) {
            if (getMark(all_users[j].student.first) == tmp[i]) {
                if (all_users[j].student.first == Mark::Empty)
                    tmp[i] = "N";
