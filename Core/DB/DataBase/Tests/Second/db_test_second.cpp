#include "db_test_second.hpp"

void db_test_second::add_test(Test& test) {
	...
	db.close();
	std::ofstream add(m_Path, std::ios::binary | std::ios::app);
...
void db_test_second::edit_test(const Test& test) {
    std::ifstream db(m_Path, std::ios::binary);
...
void db_test_second::remove_test(const Test& test) {
    // replace
...
void db_test_second::load_tests(std::vector<Test>& tests) {
    std::ifstream db(m_Path, std::ios::binary);
    db.seekg(0, std::ios::end);
